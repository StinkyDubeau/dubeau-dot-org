...
function createSighting(astro) {
    return (
        <div className="h-56 w-36 overflow-clip rounded-lg bg-lighten-800">
            <img src={astro.image} alt={astro.title} className=" h-40 w-full object-cover" />
...
export default function Astros(props) {
    return (
        <Frame>
            <p>Astros</p>
            <div className="flex flex-wrap gap-3">{astros.map(createSighting)}</div>
        </Frame>
    );
}
